"""Verify the row-cache design with trained inputs in C/RTL cosimulation."""
import argparse
import csv
import hashlib
import json
...
def main():
    ...
    p.add_argument('--width', type=int, choices=[16], default=16)
    p.add_argument('--hls-root', type=Path, default=Path('E:/use/cpu/Vivado/2019.2'))
    a = p.parse_args()
    output = ROOT / 'rtl_validation/results'
    output.mkdir(parents=True, exist_ok=True)
    if (output/'row_cache/run.log').exists(): raise ValueError('Archive old results first')
...
csynth_design
cosim_design -rtl verilog -tool xsim -trace_level none -argv "{batch.as_posix()} {(case / 'cosim_predictions.csv').as_posix()} 0"
exit
''')
        ...
        cosim = workspace / 'design/solution1/sim/report/lenet_accel_cosim.rpt'
        if not cosim.exists() or not __import__('re').search(r'Verilog\s*\|\s*Pass', cosim.read_text()):
            raise RuntimeError('RTL cosimulation did not pass; inspect run.log')
        shutil.copy2(cosim,case/cosim.name)
        for evidence in (workspace/'design/solution1/sim/report/verilog').glob('*'):
            if evidence.is_file(): shutil.copy2(evidence,case/evidence.name)
        with (case/'predictions.csv').open() as f: c_rows=list(csv.DictReader(f))
        with (case/'cosim_predictions.csv').open() as f: rtl_rows=list(csv.DictReader(f))
        if c_rows != rtl_rows: raise RuntimeError('C reference outputs differ between runs')
        tree = ET.parse(xml)
