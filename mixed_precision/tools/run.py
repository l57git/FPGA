"""Run one mixed-precision/pipeline case with isolated evidence."""
import argparse
import csv
import hashlib
import json
...
def main():
    ...
    p.add_argument('--case', choices=['w8i6_serial','w8i1_serial','w8i1_pipeline','w16i6_pipeline','w8i1_split'], required=True)
    p.add_argument('--hls-root', type=Path, default=Path('E:/use/cpu/Vivado/2019.2'))
    a = p.parse_args()
    configs = {'w8i6_serial': (8,6,0), 'w8i1_serial': (8,1,0), 'w8i1_pipeline': (8,1,1), 'w16i6_pipeline': (16,6,1), 'w8i1_split': (8,1,0)}
    width, integer, pipeline = configs[a.case]
    output = ROOT / 'mixed_precision/results' / a.case
    output.mkdir(parents=True, exist_ok=True)
    if (output/'run.log').exists(): raise ValueError('Archive this case before rerunning')
    ...
    rows = []
    for name, src in [(a.case, ROOT / ('mixed_precision/src/lenet_split.cpp' if a.case=='w8i1_split' else 'mixed_precision/src/lenet_mixed.cpp'))]:
        case = output
        case.mkdir(parents=True, exist_ok=True)
        if (case / 'predictions.csv').exists(): raise ValueError('Output exists; archive the prior experiment first')
        workspace = ROOT / 'mixed_precision/hls_work' / name
        workspace.mkdir(parents=True, exist_ok=True)
        tcl = workspace / 'run.tcl'
        tcl.write_text(f'''cd {{{workspace.as_posix()}}}
open_project -reset design
set_top lenet_accel
add_files {{{src.as_posix()}}} -cflags "-DLENET_DATA_W=16 -DWEIGHT_W={width} -DWEIGHT_I={integer} -DFC_PIPELINE={pipeline}"
add_files -tb {{{(ROOT / 'mixed_precision/tb/tb_lenet.cpp').as_posix()}}} -cflags "-DLENET_DATA_W=16 -DWEIGHT_W={width} -DWEIGHT_I={integer} -DFC_PIPELINE={pipeline}"
...
''')
        ...
        if process.returncode != 0 or not xml.exists() or 'CSim done with 0 errors' not in (case/'run.log').read_text():
            raise RuntimeError(f'{name} failed; inspect {case}/run.log')
        tree = ET.parse(xml)
        row = {'variant':name,'activation_width':16,'weight_width':width,'weight_integer':integer,'fc_pipeline':pipeline,'samples':a.samples,'tool':'Vivado HLS 2019.2','target_period_ns':10,
               'source_sha256':hashlib.sha256(src.read_bytes()).hexdigest(),
               'input_sha256':hashlib.sha256(batch.read_bytes()).hexdigest()}
        row['header_sha256'] = hashlib.sha256((ROOT/'mixed_precision/src/lenet.hpp').read_bytes()).hexdigest()
        row['testbench_sha256'] = hashlib.sha256((ROOT/'mixed_precision/tb/tb_lenet.cpp').read_bytes()).hexdigest()
