...
static data_t dense_row(const data_t input[256], const weight_t weights[256],
...
void lenet_accel(
    ...
    const weight_t conv1_w[CONV1_W_COUNT],
    const data_t conv1_b[C1_OUT],
    const weight_t conv2_w[CONV2_W_COUNT],
    const data_t conv2_b[C2_OUT],
    const weight_t fc1_w[FC1_W_COUNT],
    const data_t fc1_b[FC1_OUT],
    const weight_t fc2_w[FC2_W_COUNT],
    const data_t fc2_b[FC2_OUT],
    const weight_t fc3_w[FC3_W_COUNT],
...
#pragma HLS INTERFACE m_axi port=conv1_b offset=slave bundle=gmem0 depth=6
#pragma HLS INTERFACE m_axi port=conv2_w offset=slave bundle=gmem1 depth=2400
#pragma HLS INTERFACE m_axi port=conv2_b offset=slave bundle=gmem0 depth=16
#pragma HLS INTERFACE m_axi port=fc1_w offset=slave bundle=gmem1 depth=30720
#pragma HLS INTERFACE m_axi port=fc1_b offset=slave bundle=gmem0 depth=120
#pragma HLS INTERFACE m_axi port=fc2_w offset=slave bundle=gmem1 depth=10080
#pragma HLS INTERFACE m_axi port=fc2_b offset=slave bundle=gmem0 depth=84
#pragma HLS INTERFACE m_axi port=fc3_w offset=slave bundle=gmem1 depth=840
#pragma HLS INTERFACE m_axi port=fc3_b offset=slave bundle=gmem0 depth=10
...
POOL2_OC:
    ...
    static weight_t row_weights[256];
